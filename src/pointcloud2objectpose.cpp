...
class PointCould2Pose{
    ...
    public:
        ...
        void pointCloudCb(const PointCloud::ConstPtr& cloud_in){
            ...
            object_pose.header.frame_id="/table_top2";
            object_pose.pose.position.x=Tm(0,3);
            object_pose.pose.position.y=Tm(1,3);
            object_pose.pose.position.z=Tm(2,3);
        ...
        geometry_msgs::Quaternion quarternion_newframe(Eigen::Matrix4f& Tm,tf::Vector3& origin){
          ...
          tf_pub->sendTransform(tf::StampedTransform(transform, ros::Time::now(), "table_top2", "/Robject"+class_name));
          return geoqt;
        }
